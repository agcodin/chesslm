...
# Usage: FROM_ADAPTER=.. FROM_MODEL=.. SEED=.. ./selfplay_round_from.sh <stockfish|search> <name> [games]
...
# Start from the current best (overridable) so successive rounds build on each other.
FROM_ADAPTER=${FROM_ADAPTER:-adapters_best}; FROM_MODEL=${FROM_MODEL:-models/fused_bf16}; SEED=${SEED:-0}
...
log "collecting $games games, label=$label, from $FROM_MODEL"
[ -s $d/own/train.jsonl ] && log "reusing collected positions" || uv run selfplay_data.py --model "$FROM_MODEL" --seed "$SEED" --games "$games" --label "$label" --max-search-positions "${MAXPOS:-4000}" --out $d/own > $d/selfplay.log 2>&1
...
uv run mlx_lm.lora --model "$BASE" --train --data $d/data --iters 3000 --batch-size 16 --num-layers 16 \
  --learning-rate 7e-5 --mask-prompt --resume-adapter-file $FROM_ADAPTER/adapters.safetensors \
